...
- **Shop password reset (Resend):** add `RESEND_API_KEY` (from [Resend](https://resend.com) → API Keys). Optional: `SHOP_PASSWORD_RESET_EMAIL_FROM` like `Xtinadom Merch <password-reset@xtinadom.com>` after your domain is **verified** in Resend (DNS records at your DNS host). If `SHOP_PASSWORD_RESET_EMAIL_FROM` is unset, the app uses Resend’s test sender (`onboarding@resend.dev`) — OK for smoke tests; use your domain for real customers. Set **`NEXT_PUBLIC_APP_URL`** to your live `https://…` site so reset links in email point at Vercel, then **Redeploy**.
